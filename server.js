...
app.use((req, res, next) => {
  ...
  const info = statement.run(data.remoteaddr, data.remoteuser, data.time, data.method, data.url, data.protocol, data.httpversion, data.status, data.referer, data.useragent);
  next();
});
...
function flipACoin(call) {
  ...
  if (flipObject.call === flipObject.flip) { flipObject.result = 'win'; } 
  else { flipObject.result = 'lose'; }
  return flipObject;
}
